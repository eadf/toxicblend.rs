use toxicblend::{PB_Command, PB_Face32, PB_KeyValuePair, PB_Model32, PB_Vertex32};

use criterion::{criterion_group, criterion_main, Criterion};

fn generate_voxel_command(command_variant: String, divisions: f64) -> PB_Command {
    PB_Command {
        command: command_variant.clone(),
        options: vec![
            // used by voxel
            PB_KeyValuePair {
...
pub fn bench_fsn_voxel(c: &mut Criterion) {
    c.bench_function("bench_fsn_voxel", |b| {
        ...
        b.iter(|| {
            let _ = toxicblend::execute_command(command.clone(), false).unwrap();
        })
    });
}
