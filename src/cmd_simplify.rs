...
pub fn build_bp_model(
    ...
    for ls in lines.into_iter() {
        ...
        output_pb_model.faces.push(PB_Face{vertices:vec!(v0_mapped as i32, (v0_mapped+1) as i32)});
        let mut last_p = v0_mapped;
        for p in l.points().iter().skip(1).take(l.points().len()-2) {
            output_pb_model.faces.push(PB_Face{vertices:vec!(last_p as i32, output_pb_model.vertices.len() as i32)});
            last_p = output_pb_model.vertices.len();
            output_pb_model.vertices.push(PB_Vertex{x:p.x as f32, y: p.y as f32, z: p.z as f32});
        }
        ...
        output_pb_model.faces.push(PB_Face{vertices:vec!(last_p as i32, v1_mapped as i32)});
    }
    Ok(output_pb_model)
